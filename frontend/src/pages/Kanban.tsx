...
const columns: { id: TaskStatus; label: string; color: string; accent: string }[] = [
  { id: 'todo', label: 'To Do', color: 'bg-slate-500', accent: 'border-l-slate-500' },
  { id: 'planning', label: 'Planning', color: 'bg-blue-500', accent: 'border-l-blue-500' },
  { id: 'progress', label: 'In Progress', color: 'bg-indigo-500', accent: 'border-l-indigo-500' },
  { id: 'review', label: 'Review', color: 'bg-amber-500', accent: 'border-l-amber-500' },
  { id: 'done', label: 'Complete', color: 'bg-emerald-500', accent: 'border-l-emerald-500' },
  { id: 'cancelled', label: 'Cancelled', color: 'bg-rose-500', accent: 'border-l-rose-500' },
];

export function Kanban() {
  const { theme } = useTheme();
  const { state, addTask, updateTask, deleteTask, setTasks, setIsTaskModalOpen } = useApp();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingTask, setEditingTask] = useState<Task | null>(null);
  const [draggedTaskId, setDraggedTaskId] = useState<number | null>(null);
  const [dropTargetCol, setDropTargetCol] = useState<TaskStatus | null>(null);
  const [dropTargetIndex, setDropTargetIndex] = useState<{ col: TaskStatus; index: number } | null>(null);
  ...
  const onDrop = (e: React.DragEvent, colId: TaskStatus) => {
    ...
    const targetIndex = dropTargetIndex?.col === colId ? dropTargetIndex.index : state.tasks.filter(t => t.status === colId).length;
    
    // Update status and reorder
    const otherTasks = state.tasks.filter(t => t.id !== id);
    const colTasks = otherTasks.filter(t => t.status === colId).sort((a, b) => a.order - b.order);
        ...
        <div className="flex items-center gap-4">
          <div className="flex -space-x-2">
            ...
            <div className={cn(
              ...
              theme === 'dark' ? "border-slate-900 bg-slate-800 text-slate-300 hover:bg-slate-700" : "border-white bg-slate-100 text-slate-600 hover:bg-slate-200"
            )}>+5</div>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className={cn("flex p-1 rounded-lg border", theme === 'dark' ? "bg-slate-800 border-slate-700" : "bg-slate-100 border-slate-200")}>
            <button className={cn(
              "px-4 py-1.5 rounded-md text-sm font-semibold transition-all",
              theme === 'dark' ? "bg-slate-700 text-white shadow-sm" : "bg-white text-slate-800 shadow-sm"
            )}>Board</button>
            <button className="px-4 py-1.5 text-slate-500 hover:text-slate-800 text-sm font-medium transition-colors">Timeline</button>
            <button className="px-4 py-1.5 text-slate-500 hover:text-slate-800 text-sm font-medium transition-colors">List</button>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-6 h-full min-w-max">
          {columns.map(column => (
            <div 
              ...
              onDragOver={(e) => onDragOver(e, column.id, state.tasks.filter(t => t.status === column.id).length)}
              onDrop={(e) => onDrop(e, column.id)}
              onDragLeave={() => setDropTargetCol(null)}
            >
              <div className="flex items-center justify-between mb-4 px-1">
                <div className="flex items-center gap-2">
                  <div className={cn("w-2.5 h-2.5 rounded-sm shadow-sm", column.color)}></div>
                  <h3 className={cn("font-bold uppercase text-xs tracking-wider", theme === 'dark' ? "text-slate-200" : "text-slate-700")}>
                    {column.label}
                  </h3>
                  <span className={cn(
              ...
              <div 
                className={cn(
                  ...
                  theme === 'dark' ? "bg-slate-800/40 border-slate-700/50" : "bg-slate-100 border-slate-300/50"
                )}
                onDragOver={(e) => onDragOver(e, column.id, state.tasks.filter(t => t.status === column.id).length)}
                onDrop={(e) => onDrop(e, column.id)}
              >
                {state.tasks.filter(t => t.status === column.id).sort((a, b) => a.order - b.order).map((task, idx) => (
                  <React.Fragment key={task.id}>
                    {dropTargetIndex?.col === column.id && dropTargetIndex.index === idx && (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span className={cn(
                          ...
                          task.category === 'study' ? "bg-blue-500/10 text-blue-400 border-blue-500/20" :
                          task.category === 'work' ? "bg-purple-500/10 text-purple-400 border-purple-500/20" :
                          "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                        )}>
                          {task.category}
                        </span>
                        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openEditModal(task)} className="p-1 hover:text-indigo-500 text-slate-400"><Edit2 className="w-3 h-3" /></button>
                          <button onClick={() => deleteTask(task.id)} className="p-1 hover:text-rose-500 text-slate-400"><Trash2 className="w-3 h-3" /></button>
                        </div>
                      </div>
                      <h4 className={cn("font-semibold text-sm mb-1 transition-colors", theme === 'dark' ? "text-white group-hover:text-blue-400" : "text-slate-800 group-hover:text-blue-600")}>
                        {task.name}
                      </h4>
                      {task.note && <p className="text-xs text-slate-400 mb-3 leading-relaxed truncate">{task.note}</p>}
                ...
                {dropTargetIndex?.col === column.id && dropTargetIndex.index === state.tasks.filter(t => t.status === column.id).length && (
                  <div className="h-1 bg-indigo-500 rounded-full animate-pulse" />
                )}
                <button 
                  onClick={() => openAddModal(column.id)}
                  className={cn(
                    ...
                    theme === 'dark' ? "border-slate-700/50 hover:text-indigo-400 hover:border-indigo-500/50 hover:bg-slate-800" : "border-slate-300 hover:text-indigo-600 hover:border-indigo-500/50 hover:bg-white"
                  )}
                >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={cn(
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Task Name</label>
                <input 
                  required
                  value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Status</label>
                  <select 
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Duration (min)</label>
                  <input 
                    type="number"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Notes</label>
                <textarea 
                  value={formData.note}
