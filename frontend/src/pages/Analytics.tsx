...
export function Analytics() {
        ...
        .filter((c) => {
          if (c.isAuto && c.dateStr) {
            ...
            return (
              c.dateStr >= startStr &&
              c.dateStr <= endStr &&
              new Date(c.dateStr).getDay() === jsDay
            );
          }
  ...
  const timeAllocationData = useMemo(() => {
    ...
    return categories.map((cat) => {
      const mins = weekTasks
        .filter((t) => t.category === cat)
        .reduce((acc, t) => acc + t.duration, 0);
      return { name: cat.charAt(0).toUpperCase() + cat.slice(1), value: mins };
    });
  }, [weekTasks]);
  ...
  const productivityTrendData = useMemo(() => {
    ...
    // Show actual week days with zero scores when no history exists
    const dayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    return dayNames.map((name) => ({ name, score: 0 }));
  }, [state.scoreHistory]);
  ...
  const subjectMasteryData = useMemo(() => {
    ...
    return state.studyItems.map((item) => {
      const completed = weekTasks.filter(
        (t) =>
          t.category === "study" &&
          t.name.toLowerCase().includes(item.subject.toLowerCase()) &&
          t.status === "done",
      ).length;
      const total = weekTasks.filter(
        (t) =>
          t.category === "study" &&
          t.name.toLowerCase().includes(item.subject.toLowerCase()),
      ).length;
  ...
  const analyticsStats = useMemo(() => {
    ...
    const totalStudyMins = state.classes.reduce((acc, c) => {
    ...
    // Streak: count consecutive days with at least one done task, ending at today
    let streak = 0;
    ...
    for (let d = 0; d < 365; d++) {
      ...
      const hasDone = state.tasks.some(
        (t) => t.status === "done" && t.deadline.startsWith(dayStr),
      );
      if (hasDone) streak++;
      else break;
    }

    return { productivityPct, studyHours, doneTasks, streak };
  }, [weekTasks, state.classes, state.tasks]);
        ...
        {[
          {
            ...
            value: `${analyticsStats.productivityPct}%`,
            icon: TrendingUp,
          ...
          {
            label: "Study Hours",
            value: `${analyticsStats.studyHours}h`,
            icon: Clock,
          ...
          },
          {
            ...
            value: analyticsStats.doneTasks.toString(),
            icon: Target,
          ...
          },
          {
            ...
            value:
              analyticsStats.streak > 0
                ? `${analyticsStats.streak} Day${analyticsStats.streak !== 1 ? "s" : ""}`
                : "None",
            icon: Award,
            color: "text-rose-500",
            bg: "bg-rose-500/10",
          ...
          >
            ...
            <h3 className="text-slate-400 text-xs font-bold mb-1">
              {stat.label}
            </h3>
            <p
              className={cn(
          ...
          <div className="space-y-6">
            {subjectMasteryData.length === 0 ? (
              <div className="text-center py-10 text-slate-500 italic text-sm">
                No subjects defined. Add study items to track progress.
              </div>
            ) : (
