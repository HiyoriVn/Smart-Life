...
export function Dashboard() {
  ...
  const weekStart = useMemo(
    () => new Date(state.dateRange.currentWeekStart),
    [state.dateRange.currentWeekStart],
  );
  const weekEnd = useMemo(
    () => new Date(state.dateRange.currentWeekEnd),
    [state.dateRange.currentWeekEnd],
  );
    ...
    return [
      {
        label: "Productivity",
        value: `${productScore}%`,
        change: completePct > 0 ? `${completePct}% completed` : "No tasks yet",
        isUp: completePct > 50,
        icon: CheckCircle2,
  ...
  const alerts = useMemo(() => {
    const overdueTasks = weekTasks.filter(
      (t) =>
        t.status !== "done" &&
        new Date(t.deadline).getTime() < currentTime.getTime(),
    ).length;
    ...
    const todayDeadlines = weekTasks.filter((t) => {
      ...
      return (
        d.getTime() >= currentTime.getTime() && d.getTime() <= today.getTime()
      );
    }).length;

    if (overdueTasks > 0)
      return { type: "error", message: `⚠️ ${overdueTasks} tasks overdue!` };
    if (todayDeadlines > 0)
      return {
        type: "warning",
        message: `🔥 ${todayDeadlines} deadlines today!`,
      };
  ...
  const chartData = useMemo(() => {
    ...
    return days.map((day, i) => {
      ...
      return { name: day, value: tasksDone * 20 };
    });
  }, [weekTasks]);
  ...
  return (
    <div className="space-y-10">
      <TopBar
        title={`Welcome back, ${state.currentUser?.fullName?.split(" ")[0] || "User"}!`}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="space-y-4">
                  {todaySchedule.length === 0 ? (
                    <div className="text-center py-10 text-slate-500 text-sm italic">
                      No classes or study blocks scheduled for today.
                    </div>
                  ) : (
                    todaySchedule.map((item) => (
                      ...
                      >
                        <div
                          className="absolute left-0 top-0 bottom-0 w-1 shadow-[0_0_8px_rgba(0,0,0,0.2)]"
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <Link
                  to="/kanban"
                  className="block w-full mt-6 py-3 text-center text-xs font-bold text-slate-400 border border-dashed border-slate-600 rounded-xl hover:border-indigo-500 hover:text-indigo-500 hover:bg-indigo-500/5 transition-all"
                >
                  View All {weekTasks.filter((t) => t.status !== "done").length}{" "}
                  Pending Tasks
                </Link>
              </section>
              ...
              <section className="bg-gradient-to-br from-[#4f46e5] to-[#312e81] rounded-3xl p-6 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden border border-indigo-500/30">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-indigo-200 text-xs mb-6 max-w-[80%]">
                    Need help organizing your study plan? Let AI do the heavy
                    lifting.
                  </p>
                  <Link
                    to="/auto-scheduler"
                    className="block w-full py-3 bg-white text-center text-indigo-600 rounded-xl text-sm font-bold shadow-lg hover:bg-indigo-50 transition-colors"
                  >
                    Try Auto Scheduler
                  </Link>
                </div>
